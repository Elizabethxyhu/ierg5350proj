...
class Env_tsp():
	...
	def get_nodes(self, seed = None):
		...
		samples = torch.rand((15000, self.city_t, 4), device = device)
		for index_city in range(self.city_t):
			samples[:,index_city, 0] = samples[:,0, 0]
			samples[:,index_city, 1] = samples[:,0, 1]
		samples = samples[:,:,:]
		samples[:,:,0] = torch.ceil(7*samples[:,:,0])
		samples[:,:,1] = torch.ceil(30 * samples[:,:,1])
		samples[:,:,2] = (samples[:,:,2])*2*3.1415926
		samples[:,:,3] = (samples[:,:,3])*2*3.1415926
		return samples
	...
	def get_batch_nodes(self, n_samples, seed = None):
		...
		if seed is not None:
			torch.manual_seed(seed)
		device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
		samples = torch.rand((n_samples, self.city_t, 4), device = device)
		for index_city in range(self.city_t):
			samples[:,index_city, 0] = samples[:,0, 0]
			samples[:,index_city, 1] = samples[:,0, 1]
		samples = samples[:,:,:]
	...
	def stack_l_fast(self, inputs, tours):
		...
		*** this function is faster version of stack_l! ***
		...
		#d = torch.gather(input = inputs[:,:,:2], dim = 1, index = tours[:,:,None].repeat(1,1,2))
		feature = torch.gather(input = inputs[:,:,:], dim = 1, index = tours[:,:,None].repeat(1,1,4))
		#print('feature', feature)
		feature_edge = (feature[:, 1:] + feature[:, :-1]) / 2
		feature_edge_last = (feature[:, 0] + feature[:, -1]) / 2
		#print('feature_edge', feature_edge)
		#print('feature_edge_last', feature_edge_last.shape)
		fluctuation_edge = 10 * torch.sin(feature_edge[:,:,0]) * torch.sin(feature_edge[:,:,1]) + 100 * torch.sin(feature_edge[:,:,2]) * torch.sin(feature_edge[:,:,3]) + 110
		fluctuation_edge_last = 10 * torch.sin(feature_edge_last[:,0]) * torch.sin(feature_edge_last[:,1]) + 100 * torch.sin(feature_edge_last[:,2]) * torch.sin(feature_edge_last[:,3]) + 110
		fluctuation_edge = fluctuation_edge /110
		fluctuation_edge_last = fluctuation_edge_last /110
		#print('fluctuation_edge', fluctuation_edge)
		#print('fluctuation_edge_last', fluctuation_edge_last.shape)
		#print(fluctuation_edge)
		#fluctuation = torch.cat((fluctuation_edge, fluctuation_edge_last), 1)
		
		distance_VAR = (torch.sum(fluctuation_edge, dim = 1) + fluctuation_edge_last )   #没除110
		'''
		#print('distance_VAR', distance_VAR.mean())
		#print(d[0,0,:])
		distance_here = torch.sum((d[:, 1:] + d[:, :-1]).norm(p = 2, dim = 2), dim = 1) + (d[:, 0] + d[:, -1]).norm(p = 2, dim = 1)
		distance_here = torch.mul(distance_here, distance_VAR)
		'''
		#print('distance_here', distance_here)
		return (distance_VAR)# distance from last node to first selected node)
